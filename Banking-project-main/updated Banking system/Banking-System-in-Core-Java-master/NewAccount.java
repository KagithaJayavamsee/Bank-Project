...
public class NewAccount extends JInternalFrame implements ActionListener {
	...
	NewAccount () {
		...
		// set date controls to current date
		Calendar cal = Calendar.getInstance();
		cboMonth.setSelectedIndex(cal.get(Calendar.MONTH));
		cboDay.setSelectedItem("" + cal.get(Calendar.DAY_OF_MONTH));
		cboYear.setSelectedItem("" + cal.get(Calendar.YEAR));

		// auto-generate sequential account number and make it read-only
		String acctNo = getNextAccountNumber();
		txtNo.setText(acctNo);
	...
	// compute next account number by scanning Bank.dat for highest numeric id
	private String getNextAccountNumber() {
		long max = 0L;
		try {
			FileInputStream fis2 = new FileInputStream("Bank.dat");
			DataInputStream dis2 = new DataInputStream(fis2);
			while (true) {
				try {
					String id = dis2.readUTF();
					// skip remaining 5 fields
					for (int i = 1; i < 6; i++) {
						dis2.readUTF();
					}
					try {
			...
			// no file means no records yet
		}
		catch (IOException ioe) {
			// ignore read errors and fall back to default
		}
		if (max == 0L) {
			max = 10000000L; // starting series
		}
		long next = max + 1L;
